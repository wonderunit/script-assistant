...
const doStuff = () => {
  let scriptData = parseScript('/Users/setpixel/git/explorers-script/EXPLORERS.fountain')
  let data = renderScript(scriptData, false)
  ...
  renderScript(scriptData, true, 'test.pdf', notesScriptOptions)
}
...
const renderScript = (scriptData, render, outputFilePath, options) => {
  ...
  if (render) {
    let stream = doc.pipe(fs.createWriteStream(outputFilePath))
  }
